...
def straight_line_distance(a,b):
    return math.sqrt((a[0] - b[0])**2 + (a[1] - b[1])**2 + (a[0] + a[1] - b[0] - b[1])**2)
...
def h(coord, k, state, distance_f=uniform_cost) -> int:
    # Get blue nodes
    ...
    distances = list(map(lambda bn: max(distance_f(coord, bn) - k + 1, 1), all_blue_nodes))
